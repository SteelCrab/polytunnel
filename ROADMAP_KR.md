...
[ ] 2주차: 기준 릴리즈 패키지 및 공개 준비 진행
  [ ] `/package-spec.md`에 비컨테이너 배포 산출물 사양을 문서화한다.
  [ ] `/package-spec.md`에 아티팩트 형식, 파일명 규칙, 체크섬 정책, 설치 절차를 정의한다.
  [ ] `/package-spec.md`에 기준 플랫폼을 확정한다: linux-x86_64, linux-aarch64, linux-musl, macos-aarch64, windows-x86_64.
  [ ] `/package-spec.md`에 공개 산출물 항목(바이너리, 예시 프로젝트, 호환성 안내, 서명 정책)을 정의한다.
  [ ] README/README_KR에 체크섬 검증 기반 설치 절차를 반영한다.
  [ ] 컨테이너 배포는 v0.2.0 항목에서만 처리하고 현재는 비컨테이너 배포 기준만 확정한다.
